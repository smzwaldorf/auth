...
A local first-party identity prototype for one school. It intentionally runs as
three localhost services: one auth server and two small OIDC client examples.
...
This setup is for local development and auth-flow review. App B uses an
in-memory application session and the example secrets are local defaults; do
