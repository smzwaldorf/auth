...
if (config.CMS_ORIGIN && process.env.CMS_OIDC_CLIENT_SECRET) applications.push({ clientId: "email-cms-server", displayName: "Email CMS server", clientType: "confidential", clientSecretEnv: "CMS_OIDC_CLIENT_SECRET", publicOrigin: config.CMS_ORIGIN, redirectUris: [`${config.CMS_ORIGIN}/api/session/callback`], postLogoutRedirectUris: [`${config.CMS_ORIGIN}/login`], frontChannelLogoutUri: `${config.CMS_ORIGIN}/logout/local`, scopes: ["openid", "profile", "email", "directory:access", "offline_access"] });
try {
  await applyDirectorySeed(validateDirectorySeed({ version: 1, school: { code: "smzwaldorf", displayName: "SMZ Waldorf" }, people: [], families: [], classes: [], applications: applications.filter(app => (app.clientId === "email-cms" || app.clientId === "email-cms-server") || (process.env.DEPLOY_DEMO_APPS === "true" && (process.env.DEPLOY_PAGES !== "false" || app.clientId !== "vite-app"))), appAccess: [] }));
