...
export function familyWizard(db: Database, config: RuntimeConfig) {
  ...
  function view(d: Draft, data: Snapshot, error = "") {
    ...
    return layout("Family setup", `<style>.wizard-steps{display:flex;flex-wrap:wrap;gap:8px;padding:0;list-style:none}.wizard-steps li{padding:8px 12px;border-radius:8px;background:#edf0ea;font-size:13px}.wizard-steps [aria-current]{background:#2e5844;color:white}.wizard label{margin:16px 0}.wizard input{display:block;margin-top:6px}.family-graph{text-align:center;padding:20px 0}.graph-row{display:flex;flex-wrap:wrap;justify-content:center;gap:16px}.graph-node{background:#f5f8f2;border:1px solid #cbd8c7;border-radius:12px;padding:15px;max-width:260px;overflow-wrap:anywhere}.graph-family{margin:auto;background:#e4eee0}.graph-line{padding:12px;color:#3e604b}</style><header><div><h1>Family setup</h1><p>Nothing is saved until you confirm creation.</p></div><a href="/admin/families">Cancel setup</a></header><ol class="wizard-steps">${steps.map((s,i)=>`<li ${d.step===i+1?'aria-current="step"':''}>${i+1}. ${s}${d.familyId && i === 3 ? " (existing members)" : ""}</li>`).join("")}</ol><section class="card wizard"><h2>${d.step}. ${steps[d.step-1]}</h2>${error?`<div class="notice error" role="alert">${e(error)}</div>`:""}<form method="post" action="/admin/families/wizard"><input type="hidden" name="draft" value="${e(sign(d))}">${content}<div class="actions">${d.step>1?'<button class="secondary" name="action" value="back" formnovalidate>Back</button>':""}<button name="action" value="${d.step===6?'confirm':'next'}" formnovalidate>${d.step===6?'Confirm creation':'Continue'}</button></div></form></section>`);
  }
