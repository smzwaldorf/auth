...
For the requested clean start, Family A uses **harryworld@gmail.com** and Family B uses **smzwaldorf.education@gmail.com**. These defaults are stored in Auth `scripts/demo-families.json`; environment variables can override them. Both are seeded as parents, not administrators. The current hosted staging admin is also `smzwaldorf.education@gmail.com`, so these defaults cannot be added as hosted-demo parents on that deployment without an identity collision; use a separate demo database/deployment or choose a different controlled address. Bootstrap a separate approved administrator through the normal Auth setup after a reset. Neither address may already belong to another Auth person when applying this insert-only fixture. If sharing the deployed application, keep these clearly marked fixtures and restrict delivery to exactly the two demo addresses.
...
## Separate hosted administrator bootstrap

For a separately provisioned hosted demo, `npm run seed:demo-admin` builds an insert-only admin plan. It requires `DEMO_ADMIN_EMAIL`, distinct from both parents, and must match `STAGING_ADMIN_EMAIL` when set. No default admin address is assumed. Use the same exact `DEMO_DATABASE_CONFIRM` and secure `DATABASE_URL` as the Auth demo seed.

```sh
export NODE_ENV=production ENABLE_DEV_LOGIN=false
export DEMO_ADMIN_EMAIL='the-approved-separate-admin-inbox'
export STAGING_ADMIN_EMAIL="$DEMO_ADMIN_EMAIL"
export STAGING_PARENT_EMAILS=harryworld@gmail.com,smzwaldorf.education@gmail.com
# Configure the intended direct Auth DATABASE_URL privately and exact host/name guard.
npm run seed:demo-admin -- --check
# Only at the separately authorized hosted bootstrap stage:
npm run seed:demo-admin -- --apply
```

This creates an unverified admin with a pending login invitation; normal Google/magic-link verification is still required. It never changes an existing account or restores a revoked role. Email/ID collisions or a partial fixture abort. The existing staging admin is not migrated, demoted, or replaced. Provision normal OAuth clients separately before attempting CMS login.
