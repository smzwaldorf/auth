...
The current Cloudflare deployment is **staging**. Actions still use the historical GitHub `production` environment because it holds the live secrets; the prepared `staging` environment takes over once its secrets are populated. `NODE_ENV=production` continues enforcing hosted security settings. Do not change issuer URLs, Google callbacks, client IDs or existing secrets.
...
2. Securely provision a sending-only `RESEND_API_KEY` in the GitHub deployment environment, scoped to useaida.app where supported.
