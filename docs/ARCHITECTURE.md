...
Cloudflare Pages serves App A. Two Workers run Auth and App B through request-scoped PostgreSQL pools backed by Hyperdrive and PlanetScale Postgres, with Hyperdrive query caching disabled. App B uses encrypted durable sessions in `app_b.session` in the shared `smz-auth` database, with the same uncached Hyperdrive binding and secure host-only cookies. The shared database has a $5/month base plan. Schema separation organizes data; sharing credentials does not provide database-level privilege isolation. App A requires no database. The Node entrypoints share the same application factories.
