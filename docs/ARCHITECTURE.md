...
Cloudflare Pages serves App A. Two Workers run Auth and App B through request-scoped PostgreSQL pools backed by Hyperdrive and PlanetScale Postgres, with Hyperdrive query caching disabled. App B uses encrypted durable sessions in `app_b.session` in the shared `smz-auth` database, with the same uncached Hyperdrive binding and secure host-only cookies. The `smzwaldorf` cluster has a $5/month base plan and contains the `smz-auth` and `smz-cms` logical databases. Auth is active in `smz-auth`; `smz-cms` is currently empty. Schema separation organizes data; sharing credentials does not provide database-level privilege isolation. App A requires no database. The Node entrypoints share the same application factories.

The commit-triggered workflow validates all components. With `DEPLOY_DEMO_APPS=true` and the shared database configuration, it registers both OIDC clients, ensures the Pages project exists, and publishes all three services. Client registration never imports example people or grants directory access. Direct sign-in visits without an application request return to the launcher. Production provisioning, migration-history checks, secret rotation, backups, and real-Google release checks are documented in [Cloudflare deployment](CLOUDFLARE.md).
